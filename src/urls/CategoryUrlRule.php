...
namespace Besnovatyj\Gallery\urls;
...
class CategoryUrlRule extends BaseObject implements UrlRuleInterface
{
    ...
    public function parseRequest($manager, $request): false|array
    {
        if (preg_match('#^' . $this->prefix . '/(.*[a-z])$#is', $request->pathInfo, $matches)) {
            ...
            }, null, new TagDependency(['tags' => ['gallery_cat']]));
            ...
            if ($path != $result['path']) {
                throw new UrlNormalizerRedirectException(['Gallery/gallery/catalog/category', 'id' => $result['id']], 301);
            }

            return ['Gallery/gallery/catalog/category', ['id' => $result['id']]];
        }
        return false;
    }
    ...
    {
        if ($route == 'Gallery/gallery/catalog/category') {
            if (empty($params['id'])) {
    ...
    {
        $chunks = ArrayHelper::getColumn($category->getParents()->all(), 'slug');
